...
internally consistent.  This is the authoritative source that all
language bindings rely on.
"""

from pgwidgets_js.defs import (
...
def test_no_empty_method_names():
    """Method names should not be empty strings."""
    for name, defn in WIDGETS.items():
        for mname in defn["methods"]:
            assert mname, f"{name} has an empty method name"


def test_no_empty_callback_names():
    """Callback names should not be empty strings."""
    for name, defn in WIDGETS.items():
        for cb in defn["callbacks"]:
            assert cb, f"{name} has an empty callback name"
